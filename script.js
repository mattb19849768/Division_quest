...
function generateQuestions() {
  ...
  if (mode === 'practice') {
    // practice: each selected table ÷ 1..12
    selectedTables.forEach(t => {
      for (let b = 1; b <= 12; b++) {
        const dividend = t * b;
        questions.push({ q: `${dividend} ÷ ${t}`, expected: b, t, b });
      }
    });
 ...
 function presentQuestion() {
  ...
  if (mode === 'lightspeed' && currentQ >= questions.length - 1) {
    generateQuestions();
    currentQ = 0;
  }

  // Termination conditions
  if ((mode === 'chaos' && currentQ >= 1000000) || 
  ...
  if (multipleChoice) {
    // Multiple choice: create 4 options including the correct one
    const opts = new Set([q.expected]);
    while (opts.size < 4) {
      // add nearby numbers to the correct answer
      opts.add(Math.max(1, q.expected + Math.floor(Math.random() * 7 - 3)));
    }
